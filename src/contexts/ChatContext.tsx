...
export function ChatProvider({ children }: { children: ReactNode }) {
  ...
  const loadChatRoomMessages = useCallback(async (roomId: number) => {
    try {
      ...
      const response = await fetch(`https://www.devteam10:8080/api/chat/rooms/${roomId}/messages`, {
        method: 'GET',
        headers: {
  ...
  const refreshChatRooms = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch('https://www.devteam10:8080/api/chat/rooms/my', {
        method: 'GET',
        headers,
        credentials: 'include'
      });
